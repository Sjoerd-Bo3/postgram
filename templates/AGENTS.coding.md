...
- Treat `metadata.durable_grooming` as an operator review label until it is
  applied. Only `pgm-admin memory apply-durable-grooming --yes` turns those
  labels into rewrites or archives, and ordinary coding work should not do that
